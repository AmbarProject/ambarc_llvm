...
decl_list:
      decl {
          ...
          if ($1) p->addDeclaration(std::unique_ptr<ambar::ASTNode>($1));
          $$ = p.release();
      }
    | decl_list decl {
...
args: 
      expr { $$ = new std::vector<ambar::ASTNode*>(); if ($1) $$->push_back($1); }
    | args COMMA expr { if ($3) $1->push_back($3); $$ = $1; }
;
...
rel_expr: 
    ...
    | arith_expr EQ arith_expr { 
          auto binNode = std::make_unique<ambar::BinaryNode>("==", std::unique_ptr<ambar::ASTNode>($1), std::unique_ptr<ambar::ASTNode>($3));
          $$ = binNode.release();
      }
...
factor: 
      NUM_INT        { auto numNode = std::make_unique<ambar::NumberNode>($1); $$ = numNode.release(); }
    | NUM_REAL       { auto numNode = std::make_unique<ambar::NumberNode>($1); $$ = numNode.release(); }
    | IDENTIFIER     { auto idNode = std::make_unique<ambar::IdentifierNode>($1); free($1); $$ = idNode.release(); }
    | STRING         { auto strNode = std::make_unique<ambar::StringNode>(*$1); delete $1; $$ = strNode.release(); }
